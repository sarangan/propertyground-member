      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  //changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				else
				{
					...
					$subject = $this->config->item('website_name') . ' - ' . $this->lang->line('email_forgotten_password_subject') ;
					//$this->email->message($message);
					if ($this->email_processor->send_email($user->email, $subject, $message) )
					{
						$this->set_message('forgot_password_successful');
						return TRUE;
					}
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			else
			{
				...
				$subject =  $this->config->item('website_name') . ' - ' . $this->lang->line('email_new_password_subject');
				//$this->email->message($message);

				if ($this->email_processor->send_email($profile->email, $subject, $message) )
				{
					$this->set_message('password_change_successful');
					$this->ion_auth_model->trigger_events(array('post_password_change', 'password_change_successful'));
					return TRUE;
	...
	public function register($username, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		else
		{
			...
			else
			{
				...
				//$this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
				//$this->email->to($email);
				$subject = $this->config->item('website_name') . ' - ' . $this->lang->line('email_activation_subject');
				//$this->email->message($message);

				// if ($this->email_processor->send_email($email, $subject, $message) == TRUE)
				// {
					$this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
					$this->set_message('activation_email_successful');
					return $id;
